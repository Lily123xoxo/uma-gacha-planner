...
function calculateRolls(options) {
  ...
  if (bannerStartDate) {
      ...
      if (diffDays <= 0) return Math.floor(totalCarats / 150);

      // Monthly accumulation
      for (let i = 0; i < months; i++) {
        totalCarats += 500; // 500 carats per month when pass is purchased
      ...
      for (let i = 0; i < weeks; i++) {
        totalCarats += 110; // average 110 carats per week from login bonuses
      }
