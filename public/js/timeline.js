...
(function () {
  ...
  function formatLocalDate(s) {
    if (!s) return "Unknown";
    const d = new Date(s); return d.toLocaleDateString(undefined, { year: "numeric", month: "short", day: "numeric" });
  }
  function toYMDUTC(v) {
    if (v == null) return null;
    if (typeof v === "string") {
      const m = /^(\d{4})-(\d{2})-(\d{2})/.exec(v.trim()); if (m) return `${m[1]}-${m[2]}-${m[3]}`;
      const d = new Date(v); return isNaN(d.getTime()) ? null : d.toISOString().slice(0,10);
    }
    if (v instanceof Date && !isNaN(v.getTime())) return v.toISOString().slice(0,10);
    if (typeof v === "number" && isFinite(v)) { const d = new Date(v); return isNaN(d.getTime()) ? null : d.toISOString().slice(0,10); }
    return null;
  }
  function pickEndDateLike(b) {
    if (!b) return null;
    return b.global_actual_end_date ?? b.global_est_end_date ?? b.actual_end_date ?? b.est_end_date ?? b.end_date ?? null;
  }
  function escapeHTML(v) {
    return String(v ?? "").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");
  }
  function safeImageSrc(url) {
    try { const u = new URL(url, location.origin); if (u.origin === location.origin) return u.toString(); } catch {}
    return "/images/default.png";
  }
  function el(tag, attrs = {}, children = []) {
    const node = document.createElement(tag);
    for (const [k,v] of Object.entries(attrs)) { if (v==null) continue; if (k==="class") node.className=v; else if (k==="text") node.textContent=v; else node.setAttribute(k,v); }
  ...
  function enableDragScroll(container) {
    ...
    function onMouseMove(e){ if(!isDragging) return; const x=e.pageX-container.offsetLeft; const walk=(x-startX)*0.8; container.scrollLeft=scrollLeft-walk; }
    function onMouseUp(){ isDragging=false; window.removeEventListener("mousemove",onMouseMove); window.removeEventListener("mouseup",onMouseUp); container.style.cursor="grab"; }
    container.addEventListener("mousedown",(e)=>{ isDragging=true; startX=e.pageX-container.offsetLeft; scrollLeft=container.scrollLeft; container.style.cursor="grabbing"; window.addEventListener("mousemove",onMouseMove); window.addEventListener("mouseup",onMouseUp); e.preventDefault(); });
  }
  ...
  function toCharacterRow(b){
    if(!b) return null;
    return {
      id:b.id, uma_name:b.name ?? "", jp_release_date:b.jp_release_date ?? null,
  ...
  function toSupportRow(b){
    if(!b) return null;
    return {
      id:b.id, support_name:b.name ?? "", jp_release_date:b.jp_release_date ?? null,
      global_actual_date: b.start_kind==="actual" ? b.start_date : null,
      global_actual_end_date: b.end_date ?? null,
      global_est_date: b.start_kind==="est" ? b.start_date : null,
      global_est_end_date: b.est_end_date ?? null,
  ...
  function buildBannerThumb(b, placeholder, kind){
    ...
    const img = el("img",{ class:"banner-img", alt:imgAlt, loading:"lazy", decoding:"async", fetchpriority:"low", src:placeholder, "data-src":imgSrc });
    ...
    img.addEventListener("error", function(){ if(!this.src.endsWith(placeholder)) this.src=placeholder; });
    return section;
  }
  function buildGroupCard(index, dayGroup, placeholder){
    ...
    for(const b of all){ const ymd = toYMDUTC(b.end_date) || toYMDUTC(b.est_end_date); if(!ymd) continue; if(!groupEnd || ymd>groupEnd) groupEnd=ymd; }
    const endDate = groupEnd ? formatLocalDate(groupEnd) : "Unknown";

    const header = el("p",{ class:"mb-2 date-span", text:`${startDate} → ${endDate}` });
    const hr = el("hr",{ class:"my-2" });
  ...
  const debouncedCalculate = debounce((dayGroup)=>{ triggerCalculateFromDay(dayGroup); }, 600);
  ...
  function performSearch(query){
    ...
    if(!query){ searchMatches=[]; currentMatchIndex=-1; return; }
  ...
  async function loadTimelineGrouped(options={}){
    const { containerSelector=".timeline-scroll", endpoint="/api/banners/grouped", placeholder="/images/placeholder.png" } = options;
    ...
    try{
      ...
      let dragStartX=0, isDragging=false; const dragThreshold=15;
      container.addEventListener("mousedown",(e)=>{ dragStartX=e.clientX; isDragging=false; });
      container.addEventListener("mousemove",(e)=>{ if(Math.abs(e.clientX-dragStartX)>dragThreshold) isDragging=true; });
      ...
      container.addEventListener("click",(e)=>{
        ...
        container.querySelectorAll(".timeline-card .card").forEach(el=>el.classList.remove("selected","calculating"));
  ...
  async function initTimeline(){
    ...
    if (next)  next.addEventListener("click",()=>{ if(!searchMatches.length) return; currentMatchIndex=(currentMatchIndex+1)%searchMatches.length; scrollToCurrentMatch(); });
    if (prev)  prev.addEventListener("click",()=>{ if(!searchMatches.length) return; currentMatchIndex=(currentMatchIndex-1+searchMatches.length)%searchMatches.length; scrollToCurrentMatch(); });
